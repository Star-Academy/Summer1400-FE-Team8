...
export class PlayerComponent implements OnInit, AfterViewInit {
  ...
  separatePlayList() {
    this.updateSS(this.song.id)
      .then(() => this.editPlayLists())
      .then(() => this.showAddPage());
  }
  ...
  mousemoveRange(playRange: HTMLInputElement) {
    this.playRange.nativeElement.style.background =
      'linear-gradient(to right, #33538AFF ' + playRange.value + '%, #fff 0%)';
  }

  Process = async () => {
    await this.ngAfterViewInit();
    if (this.auth.isLogged()) this.loadSong();
    else {
  ...
  nextTrack = () => {
    ...
    this.router
      ...
      .then(() => this.router.navigate(['/player', this.recommends[this.recommand_index!].id]));
  };
  ...
  seekUpdate = async () => {
    if (this.current_track) {
      if (!isNaN(this.current_track.duration)) {
        await (this.playRange.nativeElement.value =
          this.current_track.currentTime * (100 / this.current_track.duration));
  ...
  updateplayList(item: Playlist) {
    if (!item.songs.some((e) => e.id == this.ss.id)) {
      ...
      this.playlistService.removeFromPlaylist(item.id.toString(), this.ss.id).subscribe(() =>
        this.playlistService.getPlaylists().subscribe((res) => {
          this.playLists = res;
          this.editPlayLists();
        })
